...
JSON_PATH = BASE_DIR / "processed_json" / "BMI.jsonl"
PDF_PATH = BASE_DIR / "processed_pdf" / "BMI_MHB_PO2025_chunks.jsonl"
...
for pdf_id, pdf_rec in pdf_lookup.items():
    ...
    merged = {
        ...
        "metadata": {
            ...
            "studyProgramAbbrev": pdf_meta.get("studyProgramAbbrev", "BMI"),
            "source_file": pdf_meta.get("source_file", "no data"),
            "pdf_page_start": pdf_meta.get("pdf_page_start", 0),
            "pdf_page_end": pdf_meta.get("pdf_page_end", 0),
            "studyProgram_Url": STUDIENGANG_URL,
...
print(f" Merged {len(merged_records)} BMI records → {OUT_PATH}")