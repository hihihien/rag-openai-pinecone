...
JSON_PATH = BASE_DIR / "processed_json" / "BDAISY.jsonl"
PDF_PATH = BASE_DIR / "processed_pdf" / "BDAISY_MHB_chunks_normalized.jsonl"
...
# === Step 1: Merge JSON-based modules ===
for json_id, json_rec in json_lookup.items():
    ...
    meta.update({
        ...
        "studyProgramAbbrev": pdf_meta.get("studyProgramAbbrev", meta.get("studyProgramAbbrev", "BDAISY")),
...
# === Step 2: Add unmatched PDF-only records ===
for pdf_id, pdf_rec in pdf_lookup.items():
    ...
    merged = {
        ...
        "metadata": {
            ...
            "studyProgramAbbrev": pdf_meta.get("studyProgramAbbrev", "BDAISY"),
            "source_file": pdf_meta.get("source_file", "no data"),
            "pdf_page_start": pdf_meta.get("pdf_page_start", 0),
            "pdf_page_end": pdf_meta.get("pdf_page_end", 0),
            "studyProgram_Url": STUDIENGANG_URL,
...
print(f"Merged {len(merged_records)} BDAISY records → {OUT_PATH}")
